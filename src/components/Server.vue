...
const serverOnClick = () => {
    if (store.state.server && store.state.server.city === props.server.city) {
        router.push('main').then(() => {
            proxyConnect({
                port: store.state.port,
                domain: store.state.server ? store.state.server.domain : props.server.domain,
                token: store.state.token,
                vpn_name: store.state.vpnName,
                vpn_password: store.state.vpnPassword,
            });
        });
    }
};
...
watch(server, (currentValue, oldValue) => {
    if (currentValue && ((userFree.value && !currentValue.is_paid) || !userFree.value)) {
        router.push('main').then(() => {
            proxyConnect({
                port: store.state.port,
                domain: store.state.server ? store.state.server.domain : props.server.domain,
