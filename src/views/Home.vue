                    ...
                    <li v-for="m in menus" :key="m.id">
                        <a  class="f-menu" :class="m.id == sportId ? 'active': ''">
                            <div class="toggle-sport" @click="showSecondItem(m.id)">
                                <i v-if="statusSecondItem[m.id]" class="fa-solid fa-chevron-down"></i>
                                <i v-else class="fa-solid fa-chevron-right"></i>
                            </div>
                            <div class="filter-sport" @click="setMenu(m.id)">
                                <img class="sport-icon" :src="m.icon">
                                {{m.name}}
                            </div>
                        </a>
                            ...
                            <li v-for="(s, i) in m.leagueList" :key="i" class="s-menu">
                                <a >
                                    <div class="toggle-sport" @click="showThirdItem(`${s.betradarSportId}-${i}`)" :class="categoryId == s.betradarCategoryId? 'active': ''">
                                        <i v-if="statusThirdItem[`${s.betradarSportId}-${i}`]" class="fa-solid fa-chevron-down"></i>
                                        <i v-else class="fa-solid fa-chevron-right"></i>
                                    </div>
...
<script>
    ...
    export default {
        name: "Home",
        components: {MatchDetailHighlight, MatchDetailLive, RightPanel},
        data() {
            return {
                menus: [],
                statusSecondItem: {},
        ...
        methods: {
            ...
            getSportsList() {
                let data = {}
                this.$api.getSportsList(data, (res =>{
                	if (res.data){
                        res.data.list.map((m)=> {
